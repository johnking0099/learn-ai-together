...
- **方式 A：Docker**（推荐）— 最简单，适合大多数用户
- **方式 B：Kubernetes** — 如果本机已有 K8s 环境（如 OrbStack 内置的 K8s），一条命令搞定

> **macOS 用户提示**：如果还没安装 Docker，推荐使用 [OrbStack](https://orbstack.dev/) 替代 Docker Desktop。OrbStack 启动更快、内存占用更少，且原生支持 Kubernetes 接口，完全兼容 Docker CLI。
...
如果本机已有 Kubernetes 环境（如 OrbStack 自带的 K8s），可以用一条命令完成部署。无需提前创建任何文件，所有资源（ConfigMap、Deployment、Service）通过 stdin 管道一次性创建：
...
> **注意**：K8s 方式暴露的端口是 **30888**（NodePort 范围要求 30000-32767），后续配置 MCP Server 时需要将 `SEARXNG_URL` 设为 `http://localhost:30888`。

常用管理命令：

```bash
# 查看所有 SearXNG 相关资源
kubectl get configmap,deployment,service -l app=searxng

# 查看 Pod 日志
kubectl logs -l app=searxng

# 修改配置后重新部署（重新执行上面的 kubectl apply 即可）

# 删除所有资源
kubectl delete service searxng && kubectl delete deployment searxng && kubectl delete configmap searxng-config
```
...
> **K8s 用户注意**：如果使用方式 B 部署，端口是 30888，命令改为：
> ```bash
> claude mcp add searxng -s user -e "SEARXNG_URL=http://localhost:30888" -- mcp-searxng
> ```
...
| 步骤 | 操作 | 目的 |
|------|------|------|
| 1 | 部署 SearXNG（Docker 或 K8s 二选一） | 本地搜索引擎服务 |
| 2 | `npm install -g mcp-searxng` | 安装 MCP Server |
| 3 | `claude mcp add` 添加 searxng MCP | 让 Claude Code 通过 SearXNG 搜索 |
